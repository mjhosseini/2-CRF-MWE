	...
	for p0 in possibleMults:
		...
		for p in pvariants:
			...
			for combo in y:
				...
				#combo = str(combo) # converting a nice dataAnalyzer structure to a string for processing invites bugs
				...
				for j,(word,pos) in enumerate(combo): # enumerate() makes (offset,element) tuples. you can iterate over fixed-length tuples by specifying variables for their elements.
					if pos in {'PPRON','PRONN'}: # typos in dataAnalyzer
						pos = 'PRON'
