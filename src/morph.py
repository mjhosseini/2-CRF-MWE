...
def loadMorphDataOriginalFormat():
    ...
    global morphMap
    morphMap = {}
    with gzip.open(_options.get('morphFile',os.path.dirname(os.path.abspath(__file__))+'/../dataAnalyzer/oldgaz/MORPH_CACHE.gz')) as inF:
        for ln in inF:
...
def loadMorphDataNewFormat():
    ...
    global morphMap
    morphMap = {}
    with gzip.open(_options.get('morphFile',os.path.dirname(os.path.abspath(__file__))+'/../dataAnalyzer/morph/MORPH_CACHE.gz')) as inF:
        for ln in inF:
            pos, w, stemS = ln[:-1].split('\t')
            addMorph(w, pos, stemS)
            addMorph(w, 'UNKNOWN', stemS)
