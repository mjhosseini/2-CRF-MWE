...
from classifiers2 import SVM, CRF
from classifiersD2 import CRFD
...
class DiscriminativeTagger(object):
    ...
    def _createFeatures(self, trainingData, sentIndices=slice(0,None)):
        ...
        print ("o1feats:",o1Feats)
        print (featureExtractor.__class__)
    ...
    def decode(self, nLabels, currentWeights, includeLossTerm, costAugVal, useBIO, outFile=None):
        '''
    ...
    def decode_dataset(self, dataset, print_predictions, useBIO, includeLossTerm, costAugVal, outFilePath):
        '''
        ...
        decoder = self.decode(nLabels, self._weights,
                              useBIO=useBIO, includeLossTerm=includeLossTerm, costAugVal=costAugVal, outFile=outFile)
        decoder.next()
        ...
        for sent,o0Feats in dataset:
            ...
            if print_predictions:
                # print predictions
                #J: I've changed the default print setting to be in dimsum format.
                print(sent.tagsStr())
                print()
...
def opts(actual_args=None):
    ...
    #J added:
    flag("outFilePath","Path to output file") #inflag
    flag("ctype","Classifier Type") #inflag
    flag("alpha","Regularization parameter 0-order", default=.5) #inflag
    flag("alpha2","Regularization parameter 1-order", default=.5) #inflag
    ...
    flag("itersMod", "Number of passes through the training data to write test results", ftype=int, default=1)
...
def setup(args):
    ...
    #Very good! to change to other loss function or classifier, you just need to change t and write its train and test functions.

    global featureExtractor, _args, _tagger_model, _classifierType

    #_classifierType: 0: str perc, 1: logistic regression, 2: CRF, 3: CRFD
    #J: I should'nt change featureExtractor for now, because it's just extracting features.
    #_tagger_model is t: the classifier with train and predict classes

    _classifierType = int(args.ctype)
    _alpha = float(args.alpha)
    _alpha2 = float(args.alpha2)
    print ("classifierType: ", args.ctype)
    ...
    if args.load is not None:
        ...
        if not args.disk:
            ...
            #J: training data is in this Supersense...
            ...
            if args.extract:
                ...
                if args.test_predict is not None or args.test is not None:
                    ...
                    #J: test data is in this Supersense...
                ...
                svm = SVM(_alpha)
                crf = CRF(_alpha, _alpha2,args.iters,args.itersMod,args.cutoff, evalData, args.outFilePath, args.test_predict)
                crfD = CRFD(_alpha, _alpha2,args.iters,args.itersMod,args.cutoff, evalData, args.outFilePath, args.test_predict)

                if (_classifierType == 0):
                    print ("structured perceptron")
                    #J: Here, it does training
                ...
                elif (_classifierType == 1):
                    svm.train(trainingData)
                elif(_classifierType == 2):
                    crf.train(trainingData)
                else:
                    crfD.train(trainingData)
...
#J: not sure if this will be used
...
#J:  So, this is the function to do test.
def predict(args, t, featurized_dataset=None, sentence=None, print_predictions=True):
    ...
    if args.test_predict is not None or args.test is not None:
        ...
        #J: reimplement for CRF

        if (_classifierType == 0):
            t.decode_dataset(featurized_dataset, print_predictions=(args.test_predict is not None and print_predictions),
                         useBIO=args.bio, includeLossTerm=False, costAugVal=0.0, outFilePath = outFilePath)
        else:
            t.test(featurized_dataset,outFilePath = outFilePath)


    #J: I imagine I don't care about this for now
    if args.predict is not None or sentence:
        ...
        predData = SupersenseFeaturizer(featureExtractor, dataSet,   # could be stdin, which should never be reset 
                                        t._featureIndexes, cache_features=False, domain_prefixes=args.domains)
        if (_classifierType == 0):
            t.decode_dataset(predData, print_predictions=print_predictions, useBIO=args.bio, includeLossTerm=False, costAugVal=0.0, outFilePath=outFilePath)
        else:#both svm and crf classes
            t.test(predData,outFilePath = outFilePath)
