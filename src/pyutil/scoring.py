...
class ConfusionMatrix(_ConfusionMatrix):
    ...
    def fromSets(setA, setB, others):
        '''Create a confusion matrix for binary-valued dataAnalyzer
    ...
    def fromDicts(dictA, dictB, others=set()):
        '''Create a confusion matrix for categorical dataAnalyzer given
...
def prf_plot_multiseries(series1, *args):
    '''
    Plots precision vs. recall for multiple dataAnalyzer series. Each argument of the form
    ([<PRFScores1>, <PRFScores2>, ...], <fmt(s)>, <label(s)>)
    provides precision/recall dataAnalyzer points in PRFScores instances, and optionally
