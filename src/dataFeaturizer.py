...
class SupersenseDataSet(DataSet):
    def __init__(self, path, labels, legacy0, keep_in_memory=True, autoreset=True):
        '''
        'labels' should contain all valid tags; if a tag is encountered in the dataAnalyzer
    ...
    def __iter__(self):
        '''
        Load the BIO tagged supersense dataAnalyzer from Semcor, as provided in
        the SuperSenseTagger release (SEM_07.BI).
        We also use their POS labels, which presumably were what their 
        paper used.
        One difference is that this method expects the dataAnalyzer to be converted
        into a 3-column format with an extra newline between each sentence 
        (as in CoNLL dataAnalyzer), which can be created from the SST dataAnalyzer with
        a short perl script. 
        '''
        self._reset = False
        if self._f.closed:
