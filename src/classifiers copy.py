...
class SVM:
    def train(self,trainingData):
        ...
        #print X.todense()
        clf = sklearn.linear_model.LogisticRegression()#multi_class='multinomial' another way: penalty='l1'
        print "training SVM"
        print "Y: ", Y
        clf.fit(X,Y)
        self._clf = clf
...
cdef class CRF:
    ...
    cdef double _alpha, _inf
    cdef int _nF, _nLabels, _tt
    ...
    def train(self,trainingData):
        ...
        (features, labels, X_train,Y_train,nF,nLabels,prevls,nextls,startls, endls, i2l,i2s,featNames) = self.extractXYs(trainingData)
        ...
        print "crf trained on dataAnalyzer"
    ...
    cdef fit(self,X_train,Y_train,nF,nLabels,prevls,nextls,int[:] startls, int[:] endls, i2l,i2s):
        ...
        ret = fmin_l_bfgs_b(self.compute_ll_bfgs,lmbda0,fprime=self.compute_llDer_bfgs,maxiter=10)
        print ret
        self._lmbda = ret[0]
    ...
    cdef computell(self,X_train,Y_train,double[:] lmbda,int nF,int nLabels):
        ...
        for i in range(nSamples):
            ...
            if (shouldComputeAlpha):
                ...
                alphas = self.computeAlphas(x_train_t,lmbda,nF,nLabels,i)#TODO: you can save time here!
            else:
        ...
        ret -= self._alpha * (np.linalg.norm(lmbda)**2)
        return ret
    ...
    cdef computellDerOneSample(self,x_train_t,int[:] y_train,double[:] lmbda,double[:,:] alphas,double[:,:] betas,int nW,int nF,int nLabels,double[:] prevDer):
        ...
        for j in range(curnW):
            ...
            if j!=0 and l2 in prevls[l1]:
                #print "fnumber: ",fNumber
                prevDer[fNumber] = prevDer[fNumber] + 1
        ...
        for j in range(nW):
            ...
            for ii in jidxes:
                ...
                for l1 in range(nLabels):
                    pl1 = pl1s[l1]
                    fNumber = y_idxes[ii] + nF * l1
                    prevDer[fNumber] = prevDer[fNumber] - pl1*val
            ...
            for l1 in range(nLabels):
                for l2 in prevls[l1]:
                    ...
                    prevDer[fNumber] = prevDer[fNumber] - pl1l2*val
    ...
    cdef computeDotLambdaHLabels(self,x_train_t, int l1, int l2, int j,double[:] lmbda, int nF, int nLabels):
        ...
        ret = 0
        for ii in jidxes:
            ret += lmbda[y_idxes[ii] + nF * l1] * vals[ii]
    ...
    cdef extractXYs(self, trainingData):
        ...
        _features = features.SequentialStringIndexer(cutoff=10)
        _labels = features.SequentialStringIndexer()
        _featureNames = {}
        ...
        for sent,o0FeatsEachToken in trainingData:
            ...
            for w,o0Feats in zip(sent,o0FeatsEachToken):
                ...
                wIdxCur = wIdxCur + 1
            #X_train.append(Xcur)
            X_cur_sparse = csr_matrix( (np.ones(len(rowsCur)),(rowsCur,colsCur)), shape=(wIdxCur,nF))
        ...
        return (_features, _labels, X_train_sparse,Y_train,nF,nLabels,prevls,nextls,starts, ends,i2l,i2s, _featureNames)
