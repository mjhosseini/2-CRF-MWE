...
class Trie(object):
    '''
    Trie (prefix tree) dataAnalyzer structure for mapping sequences to values.
...
def extractFirstSensePredictedLabels(sent):
    '''
    Extract most frequent sense baseline from WordNet dataAnalyzer,
    using Ciaramita and Altun's approach.  Also, uses
    the dataAnalyzer from the Supersense tagger release.
...
def loadSenseDataOriginalFormat():
    '''
    Load dataAnalyzer from the original SST release
    '''
