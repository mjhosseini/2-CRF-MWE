...
class LabeledSentence(list):
    ...
    #This is just to test a simple modle without linear dependencies
    def updatedPredictions_without_assert(self):
        '''Called once every token has a value for .prediction;
           sets .predstrength, .predlabel, and .predparent accordingly.
           Note that .predparent is 0 if no parent and a positive offset otherwise.'''
        curOuterMWE = None
    ...
    def getTag(self,tag):
        s = tag.split("-")
        return s[0]
    def getParent(self, tok,prevOffset):
        return tok.predparent
        # if tok.predparent is not None:
        #     return tok.predparent
        # else:
        #     return prevOffset
    ...
    def __unicode__(self):
        '''offset   word   lemma   POS   tag   parent   strength   label   sentID'''
        #J: I changed this to have DiMSUM format, not streusle format.

        return '\n'.join(u'{offset}\t{0.token}\t{0.stem}\t{0.pos}\t{prediction}\t{parent}\t{0.predstrength}\t{label}\t{sentId}'.format(tok,offset=i+1,sentId=self.sentId, prediction=self.getTag(tok.prediction), parent = self.getParent(tok,i), label =self.getLabel(self.getTag(tok.prediction),tok.predlabel))
                         for i,tok in enumerate(self))
