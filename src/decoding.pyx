...
def legalTagBigram(lbl1, lbl2, useBIO=False, requireClassMatch=False):
        ...
        #J: I added this:
        #Some invalid pairs:
        #invalids = ["BO","bo","BB","bb"]
        #if lbl2 is not None and lbl1 is not None:
        #    for s in invalids:
        #        if lbl1[0]==s[0] and lbl2[0]==s[1]:
        #            return False


        return True

#J: I've added a few more constraints. It wasn't in the original model, but seems necessary.
@memoize
def legalTagBigramForLogistic(lbl1, lbl2, useBIO=False, requireClassMatch=False):
        '''
        For use in decoding. If useBIO is true, valid bigrams include
          B        I
          B-class1 I-class1
...
#In the current code, c_viterbi is used.
